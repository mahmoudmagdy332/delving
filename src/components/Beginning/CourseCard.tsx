...
const CourseCard = ({ course }: { course: course }) => {
  return (
    <Link  to={`courses/${course.id}`} className="flex hover:shadow-md flex-col gap-5 items-center border rounded-2xl px-4 py-3">
      <img
        src={course.image}
  ...
  <div className="bg-blue-900 h-1 rounded-full dark:bg-blue-500" style={{width:`${course.progress}%`}}></div>
</div>
    </Link>
  );
};
