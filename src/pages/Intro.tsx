...
import { IntroQuery } from "../app/services/queries";
    ...
    <div className="my-10 w-10/12 lg:w-3/4 xl:w-1/2 mx-auto flex flex-col gap-2 items-center">
        ...
        <div className="flex items-center">
          {intro?.map((_,idx)=>(
           <>
              ...
              {idx+1<intro.length&&(
                <>
                 {idx<step?(
              <Box sx={{height:'1px',bgcolor:'primary.main'}} className="w-8 md:w-12  "></Box>
              ):idx==step?(
                <>
                <Box sx={{height:'1px',bgcolor:'primary.main'}} className=" w-4 md:w-6  "></Box>
                <Box sx={{height:'1px',bgcolor:'gray.main'}} className="  w-4 md:w-6 "></Box>
                </>
              ):(
                <Box sx={{height:'1px',bgcolor:'gray.main'}} className=" w-8 md:w-12 "></Box>
              )}
                </>
              )}
         ...
         <img src={intro?.length>0?intro[step].image:""} className="h-72"/> 
            ...
            color:'white',fontWeight:'600',mx:'30px',width:"400px",
              py:"10px",borderRadius:'7px',mt:"20px"}} >
              Continue
               </Button>
            </div>
